...
#define BLOCK_SIZE 2048   // 块大小，适配 K20c 架构
...
__global__ void block_scan(float* d_in, float* d_out, float* d_block_sums, int n) {
    ...
    // Kogge‑Stone 扫描，计算包含前缀和
    for (int offset = 1; offset < BLOCK_SIZE; offset <<= 1) {
        int src = (offset - 1) & 1;
        int dst = offset & 1;
        if (tid >= offset) {
            sdata[dst][tid] = sdata[src][tid] + sdata[src][tid - offset];
        } else {
            sdata[dst][tid] = sdata[src][tid];
        }
        __syncthreads();
    }

    int last_buf = (BLOCK_SIZE - 1) & 1;
    float inclusive = sdata[last_buf][tid];
    ...
    if (idx < n) {
        if (tid == 0)
            ...
            d_out[idx] = sdata[last_buf][tid - 1];
    }

/* harness.cu — 作业3整型前缀和评测程序（CPU 串行参考）
 *
 * 用法：./harness <N> <repeats>
 *   N       - 数组长度
 *   repeats - 计时重复次数（默认 10）
 *   variant - 13 或 26，选择固定 block 数版本（默认 13）
 *
 * 输出（单行，供 grade.py 解析）：
 *   PASS <avg_ms>
...
int main(int argc, char** argv) {
    ...
    int variant = (argc > 3) ? atoi(argv[3]) : 13;

    if (variant != 13 && variant != 26) {
        printf("FAIL bad_variant\n");
        return 0;
    }
    ...
    // ---- Warmup ----
    if (variant == 13) {
        student_prefix_sum_13block_kernel<<<13, 256>>>(d_in, d_out, n);
    } else {
        student_prefix_sum_26block_kernel<<<26, 256>>>(d_in, d_out, n);
    }
